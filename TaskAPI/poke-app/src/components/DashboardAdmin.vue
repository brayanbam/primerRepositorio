...
export default {
  ...
  async mounted() {
    ...
    if (payload.rol !== "admin") return this.$router.push("/dashboard-user");
  ...
  methods: {
    ...
    async crearTarea() {
      if (!this.usuarioSeleccionado || !this.titulo.trim()) return alert("Debe seleccionar un usuario y escribir un título.");
      try {
        const token = localStorage.getItem("token");
        ...
        this.titulo = ""; this.descripcion = ""; this.usuarioSeleccionado = ""; this.intervalo = 60;
        await this.cargarTareas();
      } catch (err) { console.error("Error creando tarea:", err.response?.data || err); alert(err.response?.data?.error || "Error creando la tarea"); }
    },
    async obtenerClima() {
      try {
